               ...
               if ($accion=="agregar") {
                ...
                //  echo $sql1;
                   $log=$_SESSION['login']."";
                      ...
                      $sql="INSERT INTO `bd_local`.`categorias_user` (`id`, `id_user`, `id_categoria`, `t_descripcion`, `estado`) VALUES ('CU-".$contador."', '".$_POST['cmbuser']."', '".$_POST['cmbtareas']."', '".$t_categoria."', 'TEMPORAL');";
//echo " el sql    ".$sql;
...
if($accion=='guardar')
{
  ...
  $contador=$contador-1;
  $sql="UPDATE `bd_local`.`categorias_user` SET `estado` = 'ACTIVO' WHERE (`id` = 'CU-".$contador."')";
//echo " el sql    ".$sql;
 $result=mysqli_query($conexion,$sql);
}
if($accion=='desabilitar')
{
 ...
 //$variablephp = "<script> document.write(getInput) </script>";
   ...
   // echo "                  ".isset($_POST['cc']);
  $sql="UPDATE `bd_local`.`categorias_user` SET `estado` = 'DESABILITAR' WHERE (`id` = '".$_POST['cc']."');";
//echo " el sql    ".$sql;
 $result=mysqli_query($conexion,$sql);
}
  ?>
              ...
              <div class="card-body table-responsive p-0" style="height: 300px;">
                <table class="table table-head-fixed text-nowrap" name="table">
                  ...
                  <thead>
                    ...
                    <?php
          $sql="SELECT cu.id as codigo , cu.t_descripcion as tarea,e.em_nombre as nombre,cu.estado as estado FROM bd_local.categorias_user as cu inner join bd_local.tbl_user as u 
inner join bd_local.tbl_emple as e inner join bd_local.tbl_categoria c 
where cu.id_user=u.us_id and c.cate_id=cu.id_categoria and e.em_id=u.em_id and cu.estado='ACTIVO' ";
                             $result=mysqli_query($conexion,$sql);
                             while($row=mysqli_fetch_assoc($result))
                          {
                               $username=$row["nombre"]."";
            ...
            ?>
                    ...
                    <tr>
                      ...
                      echo "
                       <td class='project-actions text-right'>
                          <a class='btn btn-danger btn-sm' onclick='return cargar1(\"".$row["codigo"]."\")' >
                             DESABILITAR
                          </a>
              ...
              <button type="submit" class="btn btn-primary col start" onclick="return cargar();">
